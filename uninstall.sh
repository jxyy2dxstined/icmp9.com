...
printf "2. 停止 Xray, Cloudflared 服务\n"
...
if command -v docker >/dev/null 2>&1; then
    ...
    if [ -n "$(docker images -q nap0o/icmp9)" ]; then
        docker rmi -f nap0o/icmp9:latest >/dev/null 2>&1
        info "✅ 已成功删除 nap0o/icmp9:latest 镜像"
    fi
fi

# --- 2. 清理核心业务服务 (Xray/Cloudflared/icmp9) ---
info "ℹ️ 正在清理核心业务服务 (Xray/Cloudflared/icmp9)..."
